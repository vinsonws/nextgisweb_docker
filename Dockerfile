...
# RUN add-apt-repository --yes --no-update "deb https://deb.nodesource.com/node_14.x focal main"
...
RUN . env/bin/activate && cd package && git clone https://github.com/nextgis/nextgisweb.git \
    && cd nextgisweb \
...
RUN . env/bin/activate && cd /opt/nextgis/package \
...
CMD . env/bin/activate && nextgisweb server